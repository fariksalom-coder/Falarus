...
const TASKS: SentenceTask[] = [
  {
    prompt: 'Savol tuzing: Сейчас он не слушает радио, потому что занимается.',
    words: ['почему', 'он', 'сейчас', 'не', 'слушает', 'радио', 'потому', 'что', 'занимается'],
    correct: 'почему он сейчас не слушает радио',
  },
  {
    prompt: 'Savol tuzing: Ты говоришь неправильно, потому что не учишь слова.',
    words: ['почему', 'ты', 'говоришь', 'неправильно', 'не', 'учишь', 'слова', 'потому', 'что'],
    correct: 'почему ты говоришь неправильно',
  },
  {
    prompt: 'Savol tuzing: Вы не понимаете, потому что не слушаете.',
    words: ['почему', 'вы', 'не', 'понимаете', 'не', 'слушаете', 'потому', 'что'],
    correct: 'почему вы не понимаете',
  },
  {
    prompt: 'Savol tuzing: Студенты внимательно слушают, потому что преподаватель объясняет урок.',
    words: ['почему', 'студенты', 'внимательно', 'слушают', 'преподаватель', 'объясняет', 'урок', 'потому', 'что'],
    correct: 'почему студенты внимательно слушают',
  },
  {
    prompt: 'Savol tuzing: Дети не гуляют, потому что смотрят телевизор.',
    words: ['почему', 'дети', 'не', 'гуляют', 'смотрят', 'телевизор', 'потому', 'что'],
    correct: 'почему дети не гуляют',
  },
  {
    prompt: 'Savol tuzing: Саша спрашивает, потому что не знает наш адрес.',
    words: ['почему', 'саша', 'спрашивает', 'не', 'знает', 'наш', 'адрес', 'потому', 'что'],
    correct: 'почему саша спрашивает',
  },
  {
    prompt: 'Savol tuzing: Она хорошо знает русский язык, потому что много занимается.',
    words: ['почему', 'она', 'хорошо', 'знает', 'русский', 'язык', 'много', 'занимается', 'потому', 'что'],
    correct: 'почему она хорошо знает русский язык',
  },
];
...
export default function LessonElevenTaskTwelvePage() {
  ...
  const handleNext = () => {
    if (currentIndex < TASKS.length - 1) setCurrentIndex((prev) => prev + 1);
      ...
      if (TASKS.length > 0) {
        setLessonTaskResult('/lesson-11', 13, TASKS.length, TASKS.length);
        ...
        {!finished && (
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
            <div className="space-y-1 text-center">
              <p className="text-sm font-semibold text-slate-600">Savol tuzing</p>
