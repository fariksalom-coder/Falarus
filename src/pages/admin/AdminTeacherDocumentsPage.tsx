...
export default function AdminTeacherDocumentsPage() {
  ...
  return (
    <div className="mx-auto max-w-[980px] px-4 py-6">
      <h1 className="text-[22px] font-black text-app-text">O‘qituvchilar tekshiruvi</h1>
        ...
        {TABS.map((x) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-[12.5px] font-black ${
              tab === x.key ? 'bg-slate-900 text-white' : 'bg-app-surface text-app-text-muted ring-1 ring-slate-200'
            }`}
          >
            {x.label}
          </button>
      ...
      {loading ? (
        <div className="mt-4 space-y-3">
          {[0, 1].map((i) => (
            <div key={i} className="h-40 animate-pulse rounded-2xl bg-app-surface" />
          ))}
        </div>
      ) : rows.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-app-surface px-5 py-10 text-center text-[13.5px] font-semibold text-app-text-muted">
        ...
        <ul className="mt-4 space-y-4">
          {rows.map((tt) => {
            ...
            return (
              <li key={tt.user_id} className="rounded-2xl border border-app-border bg-app-surface p-4">
                {/* Sarlavha */}
                <div className="flex flex-wrap items-center gap-3">
                  {tt.avatar_url ? (
                    <img src={tt.avatar_url} alt="" className="h-12 w-12 rounded-full object-cover" />
                  ) : (
                    <span className="flex h-12 w-12 items-center justify-center rounded-full bg-app-bg-subtle text-[13px] font-black text-app-text-muted">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[12px] font-semibold text-app-text-muted">
                      {tt.phone ?? '—'} · {tt.email ?? '—'}
                    </p>
                  </div>
                  <span
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {[
                    ...
                    <div key={k} className="rounded-xl bg-app-bg-muted px-3 py-2">
                      <p className="text-[11px] font-bold text-slate-400">{k}</p>
                      <p className="text-[12.5px] font-semibold text-app-text">{v}</p>
                    </div>
                  ))}
                </div>
                {p.about ? (
                  <div className="mt-2 rounded-xl bg-app-bg-muted px-3 py-2">
                    <p className="text-[11px] font-bold text-slate-400">O‘zi haqida</p>
                    <p className="whitespace-pre-line text-[12.5px] text-app-text">{String(p.about)}</p>
                  </div>
                ) : null}

                {/* To'lov cheki */}
                <div className="mt-3 rounded-2xl border border-app-border p-3">
                  <p className="mb-2 text-[13px] font-black text-app-text">To‘lov va chek</p>
                  {!tolov ? (
                    <p className="text-[12.5px] font-semibold text-app-text-muted">To‘lov yaratilmagan</p>
                  ) : (
                    <div className="flex flex-wrap items-start gap-3">
                      {tolov.proof_url ? (
                        ...
                        <span className="flex h-28 w-28 items-center justify-center rounded-xl bg-app-bg-muted text-center text-[11px] font-bold text-slate-400">
                          Chek yuklanmagan
                        </span>
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="text-[13px] font-black text-app-text">
                        ...
                        {tolov.status === 'pending' ? (
                          <div className="mt-2 flex gap-2">
                            ...
                            <button
                              ...
                              className="rounded-xl bg-app-surface px-3 py-2 text-[12px] font-black text-red-600 ring-1 ring-red-200 disabled:opacity-50"
                            >
                              Rad etish
                            </button>
                          </div>
                ...
                {video ? (
                  <div className="mt-3 rounded-2xl border border-app-border p-3">
                    <div className="mb-2 flex items-center gap-2">
                      <p className="text-[13px] font-black text-app-text">Video-taqdimot</p>
                      <span className="rounded-full bg-app-bg-subtle px-2 py-0.5 text-[11px] font-bold text-app-text-muted">
                        {video.status}
                      </span>
                    </div>
                ...
                {/* Hujjatlar */}
                <div className="mt-3 rounded-2xl border border-app-border p-3">
                  <p className="mb-2 text-[13px] font-black text-app-text">Hujjatlar</p>
                  {hujjatlar.length === 0 ? (
                    <p className="text-[12.5px] font-semibold text-app-text-muted">Hujjat yuklanmagan</p>
                  ) : (
                    <ul className="space-y-2">
                      {hujjatlar.map((d) => (
                        <li key={d.id} className="rounded-xl bg-app-bg-muted p-2.5">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-full bg-app-surface px-2.5 py-1 text-[11px] font-black text-app-text-muted">
                            ...
                            <span className="ml-auto text-[11px] font-black text-app-text-muted">{d.status}</span>
                          </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2 border-t border-app-border pt-3">
                  ...
                  <button
                    ...
                    className="rounded-xl bg-app-surface px-4 py-2.5 text-[12.5px] font-black text-red-600 ring-1 ring-red-200 disabled:opacity-50"
                  >
                    Profilni rad etish
                  </button>
                  <input
                    ...
                    className="min-w-[200px] flex-1 rounded-xl border border-app-border px-3 py-2 text-[12.5px]"
                  />
                </div>
              </li>
            );
      ...
      {katta ? (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => setKatta(null)}
            className="absolute right-4 top-4 h-10 w-10 rounded-full bg-app-surface/90 text-[18px] font-black text-app-text"
            aria-label="Yopish"
          >
            ✕
          </button>
          <a
            ...
            rel="noreferrer"
            onClick={(e) => e.stopPropagation()}
            className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full bg-app-surface/90 px-4 py-2 text-[12.5px] font-black text-app-text"
          >
            Yangi oynada ochish
          </a>
        </div>
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        className="rounded-lg bg-app-surface px-3 py-1.5 text-[11.5px] font-black text-red-600 ring-1 ring-red-200 disabled:opacity-40"
      >
        Rad etish
      </button>
      <input
        ...
        className="min-w-[160px] flex-1 rounded-lg border border-app-border px-2.5 py-1.5 text-[12px]"
        aria-label={`Izoh ${id}`}
      />
    </div>
  );
