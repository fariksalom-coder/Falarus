...
const TASKS: SentenceTask[] = [
  {
    prompt: 'Gapni tuzing: Он говорит правильно. Он учит слова.',
    words: ['он', 'говорит', 'правильно', 'потому', 'что', 'учит', 'слова', 'учат'],
    correct: 'он говорит правильно потому что учит слова',
  },
  {
    prompt: 'Gapni tuzing: Вечером она не гуляет. Она много работает.',
    words: ['вечером', 'она', 'не', 'гуляет', 'потому', 'что', 'много', 'работает', 'гуляют'],
    correct: 'вечером она не гуляет потому что много работает',
  },
  {
    prompt: 'Gapni tuzing: Они не обедают в университете. Они обедают дома.',
    words: ['они', 'не', 'обедают', 'в', 'университете', 'потому', 'что', 'обедают', 'дома', 'обедает'],
    correct: 'они не обедают в университете потому что обедают дома',
  },
  {
    prompt: 'Gapni tuzing: Я перевожу текст. Я знаю английский язык.',
    words: ['я', 'перевожу', 'текст', 'потому', 'что', 'знаю', 'английский', 'язык', 'знает'],
    correct: 'я перевожу текст потому что знаю английский язык',
  },
  {
    prompt: 'Gapni tuzing: Я не слушаю радио. Я делаю домашнее задание.',
    words: ['я', 'не', 'слушаю', 'радио', 'потому', 'что', 'делаю', 'домашнее', 'задание', 'делает'],
    correct: 'я не слушаю радио потому что делаю домашнее задание',
  },
  {
    prompt: 'Gapni tuzing: Он медленно читает. Он мало занимается.',
    words: ['он', 'медленно', 'читает', 'потому', 'что', 'мало', 'занимается', 'занимаются'],
    correct: 'он медленно читает потому что мало занимается',
  },
  {
    prompt: 'Gapni tuzing: Ты понимаешь текст. Ты хорошо знаешь слова.',
    words: ['ты', 'понимаешь', 'текст', 'потому', 'что', 'хорошо', 'знаешь', 'слова', 'знает'],
    correct: 'ты понимаешь текст потому что хорошо знаешь слова',
  },
];
...
export default function LessonElevenTaskElevenPage() {
  ...
  const handleNext = () => {
    if (currentIndex < TASKS.length - 1) setCurrentIndex((prev) => prev + 1);
      ...
      if (TASKS.length > 0) setLessonTaskResult('/lesson-11', 12, TASKS.length, TASKS.length);
      setFinished(true);
    }
  };
        ...
        {!finished && (
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
            <div className="space-y-1 text-center">
              <p className="text-sm font-semibold text-slate-600">Gaplarni «потому что» bilan ulang</p>
