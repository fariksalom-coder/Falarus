...
export default function DailyVocabPairsPage() {
  ...
  const onPickRight = (id: string) => {
    ...
    if (left.pairId === right.pairId) {
      ...
      setPairSelectedLeft(null);
      playCorrectSound();
      // Auto-clear the "To'g'ri!" toast so it doesn't linger while user picks the next pair.
      setTimeout(() => setPairMessage((m) => (m === "To'g'ri!" ? '' : m)), 1400);
    } else {
      ...
      <div className="min-h-screen bg-app-bg">
        <main className="mx-auto max-w-[720px] px-4 py-8 pt-[max(1rem,env(safe-area-inset-top))]">
          ...
          <div className="rounded-[20px] border border-slate-200 bg-white p-12 text-center shadow-sm">
            ...
            <p className="mt-4 text-xl font-semibold text-slate-900">{t('kunlik.pairsLockedTitle')}</p>
            <p className="mt-2 text-sm text-slate-600">{t('kunlik.pairsLockedHint')}</p>
            <button
              type="button"
