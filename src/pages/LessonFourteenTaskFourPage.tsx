...
type Task = ChoiceTask | MatchingTask | SentenceTask;
...
const TASKS: Task[] = [
  // 1️⃣ ТЕСТ — выберите правильный вариант (дать)
  { type: 'choice', prompt: 'Я ______ тебе книгу.', options: ['дашь', 'дам', 'даст', 'дадим'], correct: 'дам' },
  { type: 'choice', prompt: 'Она ______ мне ответ завтра.', options: ['дам', 'дадут', 'даст', 'дашь'], correct: 'даст' },
  { type: 'choice', prompt: 'Мы ______ вам деньги.', options: ['даст', 'дадим', 'дашь', 'дам'], correct: 'дадим' },
  { type: 'choice', prompt: 'Ты ______ мне номер телефона?', options: ['дам', 'даст', 'дашь', 'дадут'], correct: 'дашь' },
  { type: 'choice', prompt: 'Они ______ нам информацию.', options: ['даст', 'дадим', 'дадут', 'дашь'], correct: 'дадут' },
  // 2️⃣ НАЙТИ ПАРУ (дать)
  {
    type: 'matching',
    prompt: 'Juftini toping (дать)',
    pairs: [
      { left: 'Я ______ тебе совет.', right: 'дам' },
      { left: 'Ты ______ мне ответ.', right: 'дашь' },
      { left: 'Он ______ нам время.', right: 'даст' },
      { left: 'Мы ______ вам помощь.', right: 'дадим' },
      { left: 'Они ______ ребёнку подарок.', right: 'дадут' },
    ],
  },
  // 3️⃣ СОСТАВИТЬ ПРЕДЛОЖЕНИЕ
  {
    type: 'sentence',
    prompt: 'Tarjima qiling: Men senga kitob beraman.',
    words: ['я', 'дам', 'тебе', 'книгу', 'дашь', 'мы'],
    correct: 'я дам тебе книгу',
  },
  {
    type: 'sentence',
    prompt: 'Tarjima qiling: U menga javob beradi.',
    words: ['он', 'даст', 'мне', 'ответ', 'дам', 'они'],
    correct: 'он даст мне ответ',
  },
  {
    type: 'sentence',
    prompt: 'Tarjima qiling: Biz sizga yordam beramiz.',
    words: ['мы', 'дадим', 'вам', 'помощь', 'дашь', 'ты'],
    correct: 'мы дадим вам помощь',
  },
  {
    type: 'sentence',
    prompt: 'Tarjima qiling: Sen menga vaqt berasanmi?',
    words: ['ты', 'дашь', 'мне', 'время', 'дам', 'я'],
    correct: 'ты дашь мне время',
  },
  {
    type: 'sentence',
    prompt: "Tarjima qiling: Ular bolaga sovg'a berishadi.",
    words: ['они', 'дадут', 'ребёнку', 'подарок', 'даст', 'мы'],
    correct: 'они дадут ребёнку подарок',
  },
];
...
export default function LessonFourteenTaskFourPage() {
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
            <p className="text-sm font-semibold text-slate-600 text-center">
              {currentTask.type === 'choice'
                ...
                ? 'Juftini toping.'
                : 'Gapni rus tiliga tarjima qiling.'}
            </p>
            ...
            {currentTask.type === 'sentence' && status !== 'correct' && (
              <button
                ...
                onClick={() => {
                  ...
                  const ok = built === currentTask.correct || (currentTask.correct.endsWith('?') && built === currentTask.correct.slice(0, -1));
                  setStatus(ok ? 'correct' : 'wrong');
                  setMessage(ok ? 'To‘g‘ri!' : `Noto‘g‘ri. To‘g‘ri javob: ${currentTask.correct}`);
                }}
            ...
            {((currentTask.type === 'choice' && status !== 'idle') ||
