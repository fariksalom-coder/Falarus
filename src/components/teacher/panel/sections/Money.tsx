  ...
  return (
    <div>
      ...
      <Card className="mb-4 p-5">
        <p className="mb-3 text-[16px] font-semibold text-app-text">{t.incomeLast6}</p>
        <ul className="space-y-2.5">
          {(data?.months ?? []).map((m) => (
            <li key={m.month} className="flex items-center gap-3">
              <span className="w-[62px] shrink-0 text-[12px] font-medium text-app-text-muted">{m.month}</span>
              <span className="h-2.5 flex-1 overflow-hidden rounded-full bg-app-bg-subtle">
                <span
                  className="block h-full rounded-full bg-app-primary"
                  style={{ width: `${Math.round((m.total_uzs / max) * 100)}%` }}
                />
              </span>
              <span className="w-[110px] shrink-0 text-right text-[12.5px] font-semibold text-app-text">
                {fmtSum(m.total_uzs)}
              </span>
            </li>
          ))}
      ...
      <Card className="p-5">
        <p className="mb-3 text-[16px] font-semibold text-app-text">{t.incomeAdd}</p>
        ...
        {(data?.entries ?? []).length === 0 ? (
          <p className="mt-4 text-center text-[12.5px] text-app-text-muted">{t.incomeEmpty}</p>
        ) : (
          <ul className="mt-4 divide-y divide-[#F6F5FC]">
            {data!.entries.map((e) => (
              <li key={e.id} className="flex items-center gap-3 py-3">
                <span className="w-[92px] shrink-0 text-[12.5px] text-app-text-muted">
  ...
  return (
    <div>
      ...
      >
        ...
        <div className="mt-3 rounded-[14px] bg-app-surface p-3.5 shadow-[0_6px_18px_rgba(75,59,228,0.10)]">
          <p className="text-[12.5px] font-semibold text-app-text">Onlayn toʻlash</p>
          <p className="mt-1 text-[12px] leading-[1.65] text-app-text-muted">
        ...
        <div className="rounded-[14px] bg-app-surface/70 p-3.5">
          <p className="text-[12.5px] font-semibold text-app-text">
            1. {narx.toLocaleString('ru-RU')} soʻmni kartaga oʻtkazing
          </p>
          {karta ? (
            <p className="mt-1 text-[15px] font-bold tracking-wide text-app-text">
              {karta.card_number}
              <span className="ml-2 text-[12px] font-semibold text-app-text-muted">
                {karta.card_holder_name}
              </span>
            </p>
          ) : (
            <p className="mt-1 text-[12.5px] text-app-text-muted">Karta maʼlumoti yuklanmoqda…</p>
          )}
        </div>

        {/* 2-qadam: chekni yuklash */}
        <div className="mt-2.5 rounded-[14px] bg-app-surface/70 p-3.5">
          <p className="text-[12.5px] font-semibold text-app-text">2. Chekni yuklang</p>
          <p className="mt-1 text-[12px] text-app-text-muted">
            Toʻlov chekining rasmini yuklang — admin tekshirib tasdiqlaydi.
          </p>
          <label className="mt-2.5 inline-block min-h-[46px] cursor-pointer rounded-[12px] bg-app-primary px-5 py-3 text-[13px] font-semibold text-white">
          ...
          {oxirgiTolov ? (
            <p className="mt-2.5 text-[12px] font-semibold text-app-text-muted">
        ...
        <Card className="mb-4 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2.5">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="min-h-[44px] rounded-[12px] bg-app-primary px-4 py-3 text-[13px] font-semibold text-white"
            >
              Telegram
            </a>
            <a
              href={`mailto:${adminContact.email}`}
              className="min-h-[44px] rounded-[12px] border border-app-border-strong bg-app-surface px-4 py-3 text-[13px] font-semibold text-app-text"
            >
              {adminContact.email}
            </a>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <p className="mb-2 text-[16px] font-semibold text-app-text">
            {tpl(t.subFreeLessons, { used: freeUsed, limit: 3 })}
          </p>
          <p className="text-[12.5px] leading-[1.7] text-app-text-muted">
            {tpl(t.subFreeHint, { limit: 3 })}
          </p>
        </Card>

        <Card className="p-5">
          <p className="mb-3 text-[16px] font-semibold text-app-text">{t.subHistory}</p>
          {subs.length === 0 ? (
            <p className="text-[12.5px] text-app-text-muted">{t.subEmpty}</p>
          ) : (
              ...
              {subs.slice(0, 8).map((s, i) => {
                ...
                return (
                  <li key={String(row.id ?? i)} className="flex items-center gap-3 py-2.5">
                    ...
                    <span className="shrink-0 text-[11.5px] text-app-text-muted">
