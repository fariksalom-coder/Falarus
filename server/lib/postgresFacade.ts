/**
 * Минимальная PostgREST-подобная обертка для нашего backend, использующая
 * PostgreSQL через node-postgres Pool. Реализует только тот срез
 ...
 *   storage.*                 — локальное файловое хранилище в uploads/storage
 *
 * НЕ ПОДДЕРЖИВАЕТСЯ:
...
function renderSelectColumns(columns: string): string {
  ...
  if (stripped) {
    console.warn('[postgresFacade] embedded relations are not supported; stripped from select');
  }
...
function parseOrSegment(raw: string, params: unknown[]): string {
  ...
  if (!m) throw new Error(`[postgresFacade] cannot parse OR segment: "${s}"`);
  ...
  if (!(op in OP_TO_SQL)) {
    throw new Error(`[postgresFacade] OR segment op not supported: "${op}"`);
  }
...
class PgQueryBuilder<T = unknown> implements PromiseLike<QueryResult<T[]>> {
  ...
  filter(col: string, op: string, val: unknown): this {
    ...
    if (!(o in OP_TO_SQL)) {
      console.warn(`[postgresFacade] unsupported filter op "${op}" on ${col}; ignored`);
      return this;
    }
...
function safeStoragePath(root: string, bucket: string, objectPath = ''): string {
  const cleanBucket = bucket.replace(/[^a-zA-Z0-9._-]/g, '_');
  const target = path.resolve(root, cleanBucket, objectPath);
  const bucketRoot = path.resolve(root, cleanBucket);
...
class LocalStorageBucketApi {
  constructor(private root: string, private bucket: string) {}
  ...
  async upload(
    ...
    try {
      const fullPath = safeStoragePath(this.root, this.bucket, objectPath);
  ...
  getPublicUrl(objectPath: string) {
    ...
    return {
      data: {
        publicUrl: `/uploads/storage/${cleanBucket}/${cleanPath}`,
      },
    };
  }
}
...
class LocalStorageApi {
  ...
  async listBuckets() {
    try {
      ...
      return {
        data: entries.filter((entry) => entry.isDirectory()).map((entry) => ({ name: entry.name })),
        error: null,
      };
    } catch (err) {
      return {
  ...
  async createBucket(name: string, _options: { public?: boolean } = {}) {
    try {
      await fs.mkdir(safeStoragePath(this.root, name), { recursive: true });
  ...
  from(bucket: string) {
    return new LocalStorageBucketApi(this.root, bucket);
  }
}
...
class PostgresFacade {
  ...
  get auth() {
    throw new Error('[postgresFacade] auth is not supported — use the app JWT layer');
  }

  get realtime() {
    throw new Error('[postgresFacade] realtime is not supported');
  }
}

export function createPostgresFacade(pool: Pool): PostgresFacade {
  return new PostgresFacade(pool);
}
